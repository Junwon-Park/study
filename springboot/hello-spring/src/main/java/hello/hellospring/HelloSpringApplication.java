 ...
 * Gradle은 Spring boot 사용자가 설치한 애플리케이션에 필요한 라이브러리들을 찾아서 설치하고 그 라이브러리를 사용하기 위해 필요한 의존성 라이브러리들을 자동으로 찾아 설치한다.(라이브러리, 의존성 관리 툴이다.)
 * Tomcat은 웹서버(WAS: Web Application Server)이며 Spring boot는 Tomcat 서버를 내장하고 있으며 실행 시 이를 함께 실행시킨다.
 * 그렇기 떄문에 Spring boot를 실행하면 바로 기본 Port 혹은 지정한 Port로 서버가 열리며 URL을 브라우저에 입력해서 접근할 수 있게 된다.
 ...
 * 프로젝트에 변경된 내용이 있다면 새로 빌드를 해야 실행 파일에 적용된다.
